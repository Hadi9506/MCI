...
/* ---- Moving Control Variables ---- */
#define MOVE_ANGLE_OFFSET 3.0f    // 3 degrees of lean for movement
#define MOVE_MAX_PWM 300.0f       // 30% of 999 max duty cycle
#define MOVE_MIN_PWM -300.0f
#define CRASH_ANGLE_LIMIT 45.0f   // Crash detection limit

// Moving PID Gains (can be tuned separately from static balancing)
#define KP_MOVE 25.0f
#define KI_MOVE 0.50f
#define KD_MOVE 1.0f
...
volatile RobotState robot_state = STATE_STATIC;
uint8_t bt_rx_data;
...
float PID_Static(float current_angle) {
...
float PID_Moving(float current_angle, float target_setpoint) {
    static float integral = 0.0f;
    static float prev_error = 0.0f;

    // Calculate error using the offset setpoint
    float error = target_setpoint - current_angle;

    float p_term = KP_MOVE * error;
    
    integral += KI_MOVE * error * DT;
    if (integral > INTEGRAL_MAX) integral = INTEGRAL_MAX;
    if (integral < INTEGRAL_MIN) integral = INTEGRAL_MIN;

    float derivative = (error - prev_error) / DT;
    float d_term = KD_MOVE * derivative;
    prev_error = error;

    float output = p_term + integral + d_term;
    
    // Limit to 30% duty cycle for smooth, slow movement
    if (output > MOVE_MAX_PWM) output = MOVE_MAX_PWM;
    if (output < MOVE_MIN_PWM) output = MOVE_MIN_PWM;
...
int main(void)
{
  ...
  while (1)
  {
      ...
      if (huart2.RxState != HAL_UART_STATE_BUSY_RX) {
          __HAL_UART_CLEAR_OREFLAG(&huart2); // Clear Overrun flag if present
          HAL_UART_Receive_IT(&huart2, &bt_rx_data, 1);
      }
     if (display_flag == 1) {
          display_flag = 0;
          ...
          cout("State: %s\r\n", robot_state == STATE_STATIC ? "STATIC" : robot_state == STATE_FWD ? "FORWARD" : "BACKWARD");
                }
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    if (htim->Instance == TIM2) {
        ...
        if (robot_state == STATE_STATIC) {
            // Standard upright balancing
            pid_output = PID_Static(tilt_angle);
        } 
        else if (robot_state == STATE_FWD) {
            // Lean forward by 3 degrees
            float fwd_setpoint = SETPOINT + MOVE_ANGLE_OFFSET;
            pid_output = PID_Moving(tilt_angle, fwd_setpoint);
        } 
        else if (robot_state == STATE_BWD) {
            // Lean backward by 3 degrees
            float bwd_setpoint = SETPOINT - MOVE_ANGLE_OFFSET;
            pid_output = PID_Moving(tilt_angle, bwd_setpoint);
        }

        /* ==================================
         * STEP 5: DRIVE MOTORS
         * ================================== */
        if (tilt_angle > CRASH_ANGLE_LIMIT || tilt_angle < -CRASH_ANGLE_LIMIT) {
            pid_output = 0.0f;
            
            // Optional safety: reset the robot state back to static 
            // so it doesn't instantly try to drive off if you stand it back up.
            robot_state = STATE_STATIC; 
        }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    if (huart->Instance == USART2) {
        if (bt_rx_data == 'F' || bt_rx_data == 'f') {
            robot_state = STATE_FWD;
        } 
        else if (bt_rx_data == 'B' || bt_rx_data == 'b') {
            robot_state = STATE_BWD;
        } 
        else if (bt_rx_data == 'S' || bt_rx_data == 's') {
            robot_state = STATE_STATIC;
        }
        
        // Re-arm the interrupt to listen for the next command
        HAL_UART_Receive_IT(&huart2, &bt_rx_data, 1);
    }
}
