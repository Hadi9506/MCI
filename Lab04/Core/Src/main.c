...
int c=0;
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim -> Instance == TIM2) {c++;
    if (c%100==0){HAL_GPIO_TogglePin (GPIOE , GPIO_PIN_9 );}
    if (c%200==0 ){HAL_GPIO_TogglePin (GPIOE , GPIO_PIN_10 );}
    if (c%500==0){c=0;HAL_GPIO_TogglePin (GPIOE , GPIO_PIN_11 );}
  }
}
...
void cout(const char* fmt, ...) {
    ...
    va_end(args);
    if (l > 0) {
        HAL_UART_Transmit(&huart1, (uint8_t*)buffer, l, HAL_MAX_DELAY);
    }
}
